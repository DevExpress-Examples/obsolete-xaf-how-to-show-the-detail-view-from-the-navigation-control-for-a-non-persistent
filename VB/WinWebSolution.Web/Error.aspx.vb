...
Partial Public Class ErrorPage
	...
	Private Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
		...
		Dim _errorInfo As ErrorInfo = ErrorHandling.GetApplicationError()
		If _errorInfo IsNot Nothing Then
			RequestUrl.NavigateUrl = _errorInfo.Url
			RequestUrl.Text = _errorInfo.Url
			RequestUrl2.NavigateUrl = _errorInfo.Url
			RequestUrl2.Text = _errorInfo.Url
			If (Not String.IsNullOrEmpty(_errorInfo.UrlReferrer)) Then
				HyperLinkReturn.NavigateUrl = _errorInfo.UrlReferrer
			Else
				LiteralReturn.Visible = False
				HyperLinkReturn.Visible = False
			End If
			If ErrorHandling.CanShowDetailedInformation Then
				DetailsText.Text = _errorInfo.GetTextualPresentation(True)
			Else
...
'		Me.Load += New System.EventHandler(Me.Page_Load);
	...
	Protected Sub ReportButton_Click(ByVal sender As Object, ByVal e As EventArgs)
		Dim _errorInfo As ErrorInfo = ErrorHandling.GetApplicationError()
		If _errorInfo IsNot Nothing Then
			ErrorHandling.SendAlertToAdmin(_errorInfo.Id, DescriptionTextBox.Text, _errorInfo.Exception.Message)
